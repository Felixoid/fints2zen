...
from zenmoney import (
    Diff, OAuth2, Request, Transaction, ZenObjectsList, timestamp,
    ZenMoneyException
)
...
class InterTransactions(list):
    ...
    def compare_to(self, obj):
        ...
        o_only.sort()
        both.sort()
        return(InterTransactions(s_only), InterTransactions(both),
...
class FinTs(object):
    ...
    def __init__(self, blz: str, username: str, pin: str,
                 url: str, *args, **kwargs):
    ...
    def get_transactions(self, iban: str, account_uuid: str,
                         transfer_uuid: str):
        '''
...
class Zen(object):
    ...
    def __init__(self, token: str, serverTimestamp: int):
        ...
        self.diff = self.api.diff(Diff(
            serverTimestamp=serverTimestamp, forceFetch=[
                'instrument',
                'user',
            ])
        )
...
v = Validator({
    'bank': {'type': 'dict', 'required': True, 'schema': {
        'blz': {'required': True, 'type': 'integer'},
        'username': {
            'required': True, 'anyof_type': ['string', 'integer'],
        },
        'pin': {'required': True, 'type': 'string'},
        'url': {'required': True, 'type': 'string'},
    }},
    'zenmoney': {'required': True, 'type': 'dict', 'schema': {
        'token': {'required': True, 'type': 'string'},
        'withdraw_account': {'required': True, 'type': 'string'},
    }},
    'accounts': {'required': True, 'type': 'list', 'schema': {
        'type': 'list', 'minlength': 2, 'maxlength': 2, 'schema': {
...
def write_config(filename: str) -> dict:
    ...
    if bool(int(input('1=Yes, 0=No [_1_/0]: ') or 1)):
        ...
        print('You should register your application for zenmoney API,'
    ...
    print('Next ZenMoney accounts are available:\n{}'.format(
        '\n'.join(
            ['  {}: {}'.format(a[0], a[1]) for a in zip(a_titles, a_ids)]
        )
    ))

    config['zenmoney']['withdraw_account'] = str(input(
        'Enter the zenmoney account UUID for default withdraw transactions: '
    ))

    print('Enter space separated pairs of "IBAN" "zenmoney_UUID" to sync')
    while True:
        pair = [str(x) for x in
                input('empty line to stop: ').split(maxsplit=1)]
        if not pair:
            break
...
def parse_args() -> Namespace:
    default_config = os.path.join(
        ...
        os.path.join(os.path.expandvars("$HOME"), '.config'),
        'fints2zen.yaml'
    )
    parser = ArgumentParser(formatter_class=ArgumentDefaultsHelpFormatter)
    parser.add_argument(
        '-c', '--config', default=default_config, type=str,
...
def bulk_send(zen: Zen, transactions: list) -> None:
    diff = Diff(
        serverTimestamp=timestamp(), transaction=transactions
    )
    zen.api.diff(diff)
...
def serial_send(zen: Zen, transactions: list) -> None:
    for tr in transactions:
        if bool(int(input('1=Yes, 0=No [_1_/0]: ') or 1)):
            diff = Diff(
                serverTimestamp=timestamp(), transaction=[tr]
            )
            zen.api.diff(diff)
...
def main():
    ...
    zen = Zen(config['zenmoney']['token'], 1)
    bank = FinTs(config['bank']['blz'], config['bank']['username'],
                 config['bank']['pin'], config['bank']['url'])
    for pair in range(len(config['accounts'])):
        ...
        print('Pair "{} to {}".\n'
              ...
              'Already synced: {}'.format(
                  config['accounts'][pair][0], config['accounts'][pair][1],
                  len(only_bank), len(only_zen), len(both)
              ))
        if not only_bank:
            continue
        bank_to_zen = [
            Transaction(user=zen.diff.user[0].id, **i[1]) for i in only_bank
        ]
        try:
